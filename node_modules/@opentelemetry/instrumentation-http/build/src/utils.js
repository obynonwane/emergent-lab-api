...
const setSpanWithError = (span, error, obj) => {
    ...
    span.setAttributes({
        [enums_1.AttributeNames.HTTP_ERROR_NAME]: error.name,
        [enums_1.AttributeNames.HTTP_ERROR_MESSAGE]: message,
    });
    if (!obj) {
...
const getOutgoingRequestAttributesOnResponse = (response, options) => {
    ...
    if (statusCode) {
        ...
        attributes[enums_1.AttributeNames.HTTP_STATUS_TEXT] = (statusMessage || '').toUpperCase();
    }
...
const getIncomingRequestAttributesOnResponse = (request, response) => {
    ...
    const { __ot_middlewares } = request;
    ...
    const attributes = {
        ...
        [enums_1.AttributeNames.HTTP_STATUS_TEXT]: (statusMessage || '').toUpperCase(),
    };
    if (route !== undefined) {
        attributes[semantic_conventions_1.SemanticAttributes.HTTP_ROUTE] = route;
    }
    return attributes;
};
