    ...
    function patchedCallback(span, callback, _metadata) {
        const wrappedFn = (err, res) => {
            if (err) {
                ...
                span.setAttributes({
                    [enums_1.AttributeNames.GRPC_ERROR_NAME]: err.name,
                    [enums_1.AttributeNames.GRPC_ERROR_MESSAGE]: err.message,
                });
            }
            else {
        ...
        return api_1.context.bind(wrappedFn);
    }
    return (span) => {
        if (!span) {
        ...
        span.addEvent('sent');
        span.setAttributes({
            [enums_1.AttributeNames.GRPC_METHOD]: original.path,
            [enums_1.AttributeNames.GRPC_KIND]: api_1.SpanKind.CLIENT,
        });
